...
class Calendar:
	def __init__(self, year:int, month:int, day:int):
		...
		self.UI_unlocked = True # if True the user can iteract with the calendar

		self.set_UI_date("Year", year, force=True)
		self.set_UI_date("Month", month, force=True)
	...
	def update(self, events):
		...
		self.set_UI_date("Month", value=self.in_date["Month"], init=True)
		self.set_UI_date("Year", value=self.in_date["Year"], init=True)
		...
		for i in range(len(self.calendar_data)):
			...
			if not self.calendar_data[i]["Day"][0] == 0: # Write only the valid dates
				...
				if self.calendar_data[i]["Day"][0] == self.date["Day"] and self.calendar_data[i]["Month"] == self.date["Month"] and self.calendar_data[i]["Year"] == self.date["Year"]:
					...
					square_border_line(win, self.calendar_data[i]["Pos"][0],  self.calendar_data[i]["Pos"][1],  self.calendar_data[i]["Pos"][2], self.calendar_data[i]["Pos"][3], (255,0,0), 5)
					...
					try:
						for mon in range(len(self.events_data[self.in_date["Year"]][self.months_labels[self.in_date["Month"]-1]])):
							...
							if self.events_data[self.in_date["Year"]][self.months_labels[self.in_date["Month"]-1]][mon]["Day"] == self.calendar_data[i]["Day"][0]:
								square_border_line(win, self.calendar_data[i]["Pos"][0],  self.calendar_data[i]["Pos"][1],  self.calendar_data[i]["Pos"][2], 
				...
				if self.isColliderecting != None and self.UI_unlocked:
					# Rect
	...
	def add_events_data(self, year:int, monthID:int):
	...
	def add_new_event(self, day:int, year:int, monthID:int, data):
	...
	# Real date setter
	def set_real_date(self, id:str, value:int):
		self.date[id] = value

	# Setter of the month/year the user is
	def set_UI_date(self, id:str, value:int, init=False, force=False):
		if self.UI_unlocked or force:
			self.in_date[id] = value
			
			if init:
				self.intialize_grid()

	def add_UI_date(self, id:str, value:int, init=False, force=False):
		if self.UI_unlocked or force:
			self.in_date[id] += value
			...
			if init:
				self.intialize_grid()
	# Getter
	def get_UI_date(self, id:str):
		return self.in_date[id]

	def set_UI_unlocked(self, value:bool):
		self.UI_unlocked = value