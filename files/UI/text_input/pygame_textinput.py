...
class TextInput:
    ...
    def __init__(
        ...
        # Help for font test
        self.helpTestSize = self.font_object.size("test"[:0])[0]
        print("update")
    ...
    def update(self, events):
        ...
        # When there is no text, a selector shows up to make the user know where to type
        if string == "":
            self.surface = self.font_object.render(self.selector, self.antialias, self.text_color)
    ...
    def set_initial_message(self, message):
        ...
        self.cursor_position = len(message)
    ...
    def delete_keyrepeat_counter(self):
